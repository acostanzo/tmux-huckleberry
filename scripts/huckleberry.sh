...
get_tmux_option "$HUCKLEBERRY_SHORTCUT_ZOOM_KEY" "$HUCKLEBERRY_SHORTCUT_ZOOM_KEY_DEFAULT"; zoom_key="$REPLY"
get_tmux_option "$HUCKLEBERRY_SHORTCUT_ZOOM_LABEL" "$HUCKLEBERRY_SHORTCUT_ZOOM_LABEL_DEFAULT"; zoom_label="$REPLY"
get_tmux_option "$HUCKLEBERRY_SHORTCUT_ZOOM_DESC" "$HUCKLEBERRY_SHORTCUT_ZOOM_DESC_DEFAULT"; zoom_desc="$REPLY"
...
_huck_seen_keys=" "
_huck_dup=0
_huck_dup_keys=("$sessions_key" "$session_mgmt_key" "$windows_key" "$panes_key" "$find_window_key" "$buffers_key" "$toggles_key" "$config_key" "$zoom_key")
...
if [[ "$zoom_key" == "space" ]]; then zoom_display="$space_display"; else zoom_display="$zoom_key"; fi
...
_label_list=("$session_mgmt_label" "$windows_label" "$panes_label" "$find_window_label" "$buffers_label" "$toggles_label" "$config_label" "$zoom_label")
...
printf -v menu '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s' \
    ...
    "$(printf '  %s  %-*s   %s' "$zoom_display" "$max_label" "$zoom_label" "$zoom_desc")"
...
while true; do
    _expect="${sessions_key},${session_mgmt_key},${windows_key},${panes_key},${find_window_key},${buffers_key},${toggles_key},${config_key},${zoom_key}"
    ...
    if [[ "$key" == "$sessions_key" ]]; then
        ...
        if [[ "$selection" == *"$sessions_label"* ]]; then
            ...
            palette="config"
        elif [[ "$selection" == *"$zoom_label"* ]]; then
            tmux resize-pane -Z
            exit 0
